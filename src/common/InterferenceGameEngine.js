...
export default class InterferenceGameEngine extends GameEngine {
    ...
    if (this.notesByRoom[r]) {
      for (let i = 0; i < this.notesByRoom[r].length; i++) {
        ...
        for (let j = i + 1; j < this.notesByRoom[r].length; j++) {
          ...
          if (
            this.notesByRoom[r][i].xPos === this.notesByRoom[r][j].xPos &&
  ...
  quantizedPosition(x, y, divX, divY, roomName) {
    let cellX =
      Math.floor(x / (this.paramsByRoom[roomName].playerWidth / divX)) *
      (this.paramsByRoom[roomName].playerWidth / divX);
    let cellY =
      Math.floor(y / (this.paramsByRoom[roomName].playerHeight / divY)) *
      (this.paramsByRoom[roomName].playerHeight / divY);
    return [cellX, cellY];
  }
  ...
  setRoomParamsToDefault(room) {
    ...
    Object.assign(this.paramsByRoom[room], {
      ...
      eggSoundsToUse: ["melody", "bass", "perc"],
      numStartingEggs: 2,
      numEggsToAdd: 1,
      ballWraps: true,
      eggHPRange: 0,
      ...
      leftBound: 0,
      topBound: 0,
      eggDroneVolume: -8, // in decibels
      transportSyncInterval: 180,
      eggRadius: 1,
      eggBaseVelocity: 0.1,
      ammoDropChance: 0.025,
      actionThreshold: 8,
